...
# 0. Set-up
# 1. For each feature class, update the attribute table fields of CLASS, CROP_TYPE, and GIS_COMMENTS, and STUDY_AREA for any features with matching FIELD_ID and REGION 
...
for i in earlier_feature_classes.split(";"): 
    ...
    # Make empty list of field names matching case-insensitive element from desired_field_names
    field_names_list = []
    
    # For each existing field (ignoring case sensitivity), check if field name is in the DesiredNames list, and add to field_names list if so
    for e in existing_fields:
        for d in desired_field_names:
            ...
            if e_insensitive[0:6] == d_insensitive[0:6]:
                print(e_insensitive[0:6])
                print(d_insensitive[0:6])
